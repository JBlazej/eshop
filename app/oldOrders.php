...
$stmt = $db->prepare("SELECT NumberOrder, SUM(Amount) as PrP, DATE_FORMAT(Date,'%d/%m/%Y') AS PrD, TotalPrice FROM Orders2 WHERE Ok = 1 GROUP BY NumberOrder ORDER BY Date DESC LIMIT 15 OFFSET ?");
  ...
  <div id="container">
    ...
    <a href="star.php"><h3>Statistics</h3></a>
     ...
     <?php foreach($ords as $row) { ?> 
    	...
    	<a class="<?= $offset/15+1==$i ? "active" : ""  ?>" href="oldOrders.php?offset=<?= ($i-1)*15 ?>"><?= $i ?></a>
		<?php } ?>
